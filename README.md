...
The submitted project report is available in [`project-report.pdf`](./project-report.pdf), and the assignment brief is available in [`assignment.pdf`](./assignment.pdf). Both documents are written in Portuguese.
...
- 🖥️ Console output with one readable summary per packet.
...
- 🎛️ Friendly CLI filters for IP, MAC, protocol, ports, and IPv4 fragmentation.
...
assignment.pdf       Assignment brief in Portuguese
```
